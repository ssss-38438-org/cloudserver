...
/*
GET /_/backbeat/raftmetadata/<raft session id>/bucket
GET /_/backbeat/raftmetadata/<raft session id>/bucket/<bucket name>/object/
GET /_/backbeat/raftmetadata/<raft session id>/bucket/<bucket name>/object/ +
    <object key>
...
// TODO: one wrapper function for all the metadata calls so no repeat code
...
function routeBackbeat(clientIP, request, response, log) {
    ...
    return async.waterfall([next => auth.server.doAuth(
        ...
        (bucketInfo, objMd, next) => {
            ...
            console.log('calling backbeatRoutes', backbeatRoutes);
            return backbeatRoutes[request.method][request.resourceType](
